...
const orderHandler = async (
    ...
    res: Res
      ...
      try { 
          const data:any = await initRequest.OrderList(req.query);
          switch (data && isAuth) {
              case data && isAuth:
                const formatedData = data.map((n:any) => (n.orderedItems ? {...n, orderedItems:JSON.parse(n.orderedItems)} : n.orderedItems = null));
                    res.status(200).json({
                        success:true,
                        message:"Success!",
                        data:formatedData
...
export default getMiddleware(orderHandler)
