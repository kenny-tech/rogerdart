...
const paymentHistoryDetails = async (
  ...
  if(!req.headers[`authorization`]) return res.status(401).json({success:false, message:`Unauthorised: You must be signed in.`});
    ...
    try {
      ...
      switch (isAuth) {
        case isAuth:
          const data:any =  await initializeRequest.paymentHistoryDetails(req.query.id, req.query.paymentId);
          res.status(200).json({
            success:true,
            message: "Your payment details",
            data:data[0]
          })
          break;
...
export default getMiddleware(paymentHistoryDetails)