...
const identityVerification = async (
  ...
  if(!req.headers[`authorization`]) return res.status(401).json({success:false, message:`Unauthorised: You must be signed in.`});
    ...
    try {
      switch (isAuth) {
        case isAuth:
          const data =  await initializeRequest.identityVerification(req.query.id, req.body);
          res.status(200).json({
            success:true,
            response:data ? [] : data,
            message:`Enter the 6 digit OTP sent to your number ending with ...${req.body.owner_mobile.slice(-3)}`,
          })
          break;
...
export default putMiddleware(identityVerification)