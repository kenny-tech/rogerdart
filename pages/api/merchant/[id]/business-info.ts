...
const updateBusinessInformationHandler = async (
  req: NextApiRequest,
  res: NextApiResponse
) => {
  if(!req.headers[`authorization`]) return res.status(401).json({success:false, message:`Unauthorised: You must be signed in.`});
    ...
    try {
      switch (isAuth) {
        case isAuth:
          const data =  await initializeRequest.updateBusinessInformation(req.query.id, req.body);
          res.status(200).json({
            success:true,
            // business_info:data,
...
export default putMiddleware(updateBusinessInformationHandler)