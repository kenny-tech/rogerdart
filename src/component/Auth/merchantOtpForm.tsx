...
import { RESET_PASSWORD_MERCHANT_PAGE_ROUTE, PUBLIC_BASE_URL, VALIDATE_OTP } from '@src/services/routes';
import Head from 'next/head';
import { PageInfo } from "@src/component";
import { errorAlert } from "@src/services/alert";

const MerchantOtpForm = () =>{
       ...
       function handleCodeSubmission(){
        ...
        if(email){
            ...
            .then(res=>{
                ...
                router.push({
                    pathname:`${RESET_PASSWORD_MERCHANT_PAGE_ROUTE}`, 
        ...
        <div>
            ...
            <div className={verifyCodeStyles.digitGroup}>
                <div className="card border-0 p-3 mb-5 bg-white rounded">
                    ...
                    <hr style={{width:'400px', height:'3px', backgroundColor: '#E0E0E0', border: 'none'}}/>                        
                    {
                        isLoading ? (<button className={verifyCodeStyles.verifyMerchantOtpButton} type='submit' disabled={true}>
                                    <p className="pt-3">Confirming...</p>
                        </button>) : (<button className={verifyCodeStyles.verifyMerchantOtpButton} type='submit' onClick={handleCodeSubmission}>Confirm</button>) 
                    }
                </div>
            </div>
        </div>
